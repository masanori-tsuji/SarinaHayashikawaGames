...
LPDIRECT3DTEXTURE9 CContinue::m_pTexture[TYPE_MAX] = {};
CContinue::CContinue()
{
	m_pVtxBuff = NULL;
	m_nButtonPos = 1;
...
CContinue * CContinue::Create(D3DXVECTOR3 pos, float fSizeWidth, float fSizeHeight)
{
	CContinue *pPause;
...
HRESULT CContinue::Load(void)
{
	CRenderer * pRenderer = CManager::GetRenderer();
	LPDIRECT3DDEVICE9 pDevice = pRenderer->GetDevice();
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, TEXTURE_CONTINUE, &m_pTexture[TYPE_BG]);
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, TEXTURE_BUTTON_CONTINUE, &m_pTexture[TYPE_BUTTON_001]);
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, TEXTURE_BUTTON_END, &m_pTexture[TYPE_BUTTON_002]);
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, TEXTURE_CONTINUE_UI, &m_pTexture[TYPE_UI]);
	return S_OK;
}

void CContinue::Unload(void)
{
...
HRESULT CContinue::Init(D3DXVECTOR3 pos, float fSizeWidth, float fSizeHeight)
{
	CRenderer * pRenderer = CManager::GetRenderer();
	LPDIRECT3DDEVICE9 pDevice = pRenderer->GetDevice();
...
#define JS
		...
		if (lpDIDevice != NULL && pInputJoystick->GetJoystickTrigger(JS_A)||pInputKeyboard->GetKeyboardTrigger(DIK_SPACE))//決定
		{
			//ショット音の再生
			...
			{
			case 1://ゲーム戻るボタン
				CGame::SetIsStopUpdateContinue(false);
				pPlayer->AddLife(PLAYER_INITLIFE);
				pPlayer->AddBomb(PLAYER_INITLIFE);
				m_nButtonPos = 1;
				break;
			case 2://最初からボタン
